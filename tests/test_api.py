...
class TestTaskAPI:
    ...
    def test_filter_tasks_by_status(self, auth_client, project):
        Task.objects.create(title="Pending", project=project, status=Task.Status.PENDING)
        Task.objects.create(title="Completed", project=project, status=Task.Status.COMPLETED)
